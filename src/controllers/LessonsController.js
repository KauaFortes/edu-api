...
exports.findById = async (req, res) => {
  try {
    ...
    // Define um avatar padrão se n existir
...
exports.delete = async (req, res) => {
    try {
      ...
      if (!lesson) {
        return res.status(404).send({ status: `lesson com o ${id} não encontrado `})
      }
      ...
      return res.status(204).send({ status: 'A lesson foi deletada com sucesso'})
    } catch (e) {
      return res.status(500).send({ error: e.message || e });
    }
  }
